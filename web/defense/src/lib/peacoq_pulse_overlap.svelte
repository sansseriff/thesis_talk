<script lang="ts">
    ...
    const dyno_nav = {
        async pulse_1_in() {
            ...
            slider_value_1.set(1050);
        },
        async pulse_1_left() {
            // move the first pulse to the left
            slider_value_1.set(800);
        },
        async pulse_1_right() {
            // move the first pulse to the right
            slider_value_1.set(1100);
        },
        async pulse_2_in() {
            slider_value_2.set(700);
        },
        async pulse_2_left() {
        ...
        action_state: 0,
        left: async function () {
            console.log(this.action_state)
            if (this.action_state > 0) {
                this.action_state -= 1;
            } else {
                this.action_state = 0;
            }
            let action = actions[this.action_state];
        ...
        right: async function () {
            console.log(this.action_state)
            if (this.action_state < actions.length - 1) {
                this.action_state += 1;
            } else {
                this.action_state = actions.length - 1;
            }
            let action = actions[this.action_state];
            // check if action is a string or a function
            if (typeof action === "string") {
    ...
    const actions = [
        prev_slide,
        dyno_nav.pulse_1_in,
        dyno_nav.pulse_1_left,
        dyno_nav.pulse_1_right,
        dyno_nav.pulse_2_in,
        dyno_nav.pulse_2_left,
        dyno_nav.pulse_2_right,
        next_slide,
    ];

    console.log(actions[3])

    const slider_value_1 = tweened(700, {
        duration: 1500,
        easing: cubicInOut,
    });
    ...
    async function make_graph(json_data: JsonData) {
        ...
        setTimeout(() => {
            ...
            abs.style.left =
                targetRect.left + (2 * targetRect.width) / 3 + "px";
