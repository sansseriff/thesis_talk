  ...
  onMount(() => {
    ...
    keydownHandler = async function(event) {
      ...
      if (event.key === 'ArrowLeft' || event.key === 'PageUp') {
        ...
        navigate('/slide_124', '/slides_png/slide_124.png');
      }
      //Check if the pressed key is the right arrow key
      else if (event.key === 'ArrowRight' || event.key === 'PageDown') {
        // Navigate to the desired URL when the right arrow key is pressed
        navigate('/slide_126', '/slides_png/slide_126.png');
      }

      else if (event.key === '.') {
        // Navigate to the backup slide
        navigate('/slide_125_backup_3', '/slides_png/slide_125_backup_3.png');
      }
    };
...
<div id="floating-number">125</div>
<img src="/slides_png/slide_125_backup_2.png" alt="slide_125_backup_2" width="{$width}">
